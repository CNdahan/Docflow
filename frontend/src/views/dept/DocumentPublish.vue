  ...
  MENU_CONF: {
    uploadImage: {
      async customUpload(file: File, insertFn: (url: string) => void) {
        try {
          const r = await attApi.uploadAttachment({ ownerType: 'INLINE', file });
...
function onCreated(e: IDomEditor) { editorRef.value = e; }

interface UploadedAtt { id: number; file_name: string; size_bytes: number; }
...
async function doUpload(file: File, purpose: 'READING' | 'TEMPLATE') {
  ...
  try {
    const r = await attApi.uploadAttachment({ ownerType: 'DOCUMENT_DRAFT', purpose, file });
...
async function removeAtt(id: number, type: 'reading' | 'template') {
  ...
  if (type === 'reading') readingAtts.value = readingAtts.value.filter((a) => a.id !== id);
  else templateAtts.value = templateAtts.value.filter((a) => a.id !== id);
}
...
async function onSubmit() {
  if (!canSubmit.value) { ElMessage.warning('请完整填写表单'); return; }
  submitting.value = true;
  try {
    const doc = await docsApi.publishDocument({
      title: title.value.trim(),
      content_html: content.value,
      target_scope: 'OWN_DEPARTMENT',
    ...
    router.push(`/dept/documents/${doc.id}`);
  ...
  <el-card>
    ...
    <el-form label-position="top" style="margin-top: 16px">
      ...
      <el-form-item label="发送范围">
        ...
        <span style="color: #909399; margin-left: 8px; font-size: 13px">自动发送给本部门所有普通用户</span>
      </el-form-item>
...
.att-row { display: flex; justify-content: space-between; align-items: center; padding: 4px 0; border-bottom: 1px dashed #ebeef5; }
