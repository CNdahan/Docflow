...
async function reload() {
  loading.value = true;
  try {
    const resp = await api.listDocuments({ page: page.value, size });
...
<template>
  <el-card>
    <div class="toolbar">
      <h3 style="margin: 0">公文管理</h3>
      <el-button type="primary" @click="router.push('/dept/documents/new')">+ 发布公文</el-button>
    </div>

    <el-table :data="list" v-loading="loading" border style="margin-top: 16px"
      @row-click="(row: DocumentItem) => router.push(`/dept/documents/${row.id}`)">
